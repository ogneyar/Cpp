...
int main()
{
  ...
  // #endif
       ...
       while ( true )
    {
        ...
        const int sent_bytes = sendto( handle, (const char*)packet_data, maximum_packet_size,
                             0, (sockaddr*)&address, sizeof(sockaddr_in) );

        if ( sent_bytes != maximum_packet_size )
        {
...
int create_socket(const char *apstrPort)
{
  ...
  address.sin_family = AF_INET;
  address.sin_addr.s_addr = INADDR_ANY;
  address.sin_port = htons( (unsigned int) *apstrPort );
